    ...
    try {
        ...
        $dup = $db->prepare(
            "SELECT patient_id FROM patients 
             WHERE full_name=? AND birthday=? AND contact_number=? LIMIT 1"
        );
        $dup->execute([
            $_POST['full_name'],
            $_POST['birthday'],
        ...
        if ($dup->rowCount() > 0) {
            throw new Exception("Duplicate patient detected.");
        }
        ...
        // Add checkup including doctor info
        ...
        $db->commit();
        header("Location: doctor_patient_data_management.php?success=1");
        exit();
...
<div class="card section-card mb-4 shadow-sm">
<div class="section-header">
    <i class="fa-solid fa-stethoscope me-2"></i> Initial Checkup
</div>
<div class="card-body">
